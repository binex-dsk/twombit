...
void Color::update() {
    ...
    for(i = 0; i < 64; i++) {
		...
		colorTable[Mode::SMS]._RGB32[i] = (r << 16) | (g << 8) | (b);

		r >>= 3;
		g >>= 2;
		b >>= 3;
		colorTable[Mode::SMS]._RGB16[i] = (r << 11) | (g << 5) | (b);
	}
	...
	for(i = 0; i < 16; i++) {
		...
		colorTable[Mode::TMS]._RGB32[i] = (r << 16) | (g << 8) | (b);

		r >>= 3;
		g >>= 2;
		b >>= 3;
		colorTable[Mode::TMS]._RGB16[i] = (r << 11) | (g << 5) | (b);
	}
	...
	for(i = 0; i < 16; i++) {
		...
		colorTable[Mode::SMS_TMS]._RGB32[i] = (r << 16) | (g << 8) | (b);

		r >>= 3;
		g >>= 2;
		b >>= 3;
		colorTable[Mode::SMS_TMS]._RGB16[i] = (r << 11) | (g << 5) | (b);
	}
    ...
    for(i = 0; i < 4096; i++) {
        ...
        colorTable[Mode::GG]._RGB32[i] = (r << 16) | (g << 8) | (b);

        r >>= 3;
        g >>= 2;
        b >>= 3;
        colorTable[Mode::GG]._RGB16[i] = (r << 11) | (g << 5) | (b);
    }
